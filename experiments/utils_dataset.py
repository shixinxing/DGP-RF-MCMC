...
def load_UCI_dataset(dataset_name, batch_size=128, transform_fn=None, data_dir='./data/', drop_train_remainder=True):
    ...
    # drop remainder to prevent a small final batch.
    # if batch is too large, drop remainder=True will return the no data
    ds_train = ds_train.batch(batch_size, drop_remainder=drop_train_remainder)
    ds_test = ds_test.batch(batch_size, drop_remainder=False) # if batch size is too large, will return the whole data
