...
from utils_dataset import load_UCI_dataset, download_UCI_data_info, load_tf_dataset, normalize_MNIST
from utils import cyclical_step_rate


def regression_train(model, dataset_name='boston', batch_size=200, data_dir='./data/',
                     lr_0=0.01, momentum_decay=0.9, full_bayesian=True,
    ...
    if train_size_drop_remainder == 0: # batch size > train size
        print("Training size is 0 after remainder dropping! Using the whole data as one batch! ")
        ds_train, ds_test, train_size_drop_remainder, test_shape = load_UCI_dataset(dataset_name, batch_size=batch_size,
                                                                                    data_dir=data_dir,
                                                                                    drop_train_remainder=False)
    ...
    for epoch in range(total_epochs):
        model.precond_update(ds_M, train_size, K_batches=K_batches, full_bayesian=full_bayesian,
        ...
        for img_batch, label_batch in ds_train:
            ...
            if epoch < start_sampling_epoch: # fixed learning rate, zero temperature
                model.sgmcmc_update(img_batch, label_batch, train_size,
                                    lr=lr_0, momentum_decay=momentum_decay, full_bayesian=full_bayesian,
                ...
                if resample_in_cycle_head == True:
                    ...
                    is_new_cycle = False
                model.sgmcmc_update(img_batch, label_batch, train_size,
                                    lr=lr, momentum_decay=momentum_decay, full_bayesian=full_bayesian,
                ...
                if is_end: # sampling the model
                    ...
                    log_p.append(test_log_p - tf.math.log(Y_std)) # restore via Y_std
                    mse.append(test_se * Y_std**2.)
        ...
        if (epoch + 1) % print_epoch_cycle == 0:
            ...
            test_log_p, test_se = model.eval_log_likelihood_and_se(ds_test)
            print(f"Epoch: {epoch}")
            print(f"Mean Log Likelihood -- train: {tf.reduce_mean(train_log_p) - tf.math.log(Y_std)}, "
                  f"-- test: {tf.reduce_mean(test_log_p) - tf.math.log(Y_std)} ")
...
def classification_train(model, dataset_name='mnist', batch_size=200, data_dir='./tensorflow_datasets/',
                     lr_0=0.01, momentum_decay=0.9, full_bayesian=True,
                     precond_type='identity', K_batches=None, second_moment_centered=None,
                     resample_in_cycle_head=False,
                     total_epochs=5000, start_sampling_epoch=2000, epochs_per_cycle=50,
    ...
    if precond_type != 'identity' and K_batches is None and second_moment_centered is None:
        raise ValueError("Args K_batches or second_moment_centered shouldn't be None!")
    # ds_train has dropped remainder
    ds_train, ds_test, train_full_size, test_full_size = load_tf_dataset(dataset_name, transform_fn=normalize_MNIST,
                                                                         batch_size=batch_size, data_dir=data_dir)
    train_size_drop_remainder = train_full_size - train_full_size % batch_size
    if train_size_drop_remainder == 0:
        print("Training size is 0 after remainder dropping! Using the whole data as one batch! ")
        ds_train, ds_test, train_size_drop_remainder, test_shape = load_UCI_dataset(dataset_name, batch_size=batch_size,
                                                                                    data_dir=data_dir,
                                                                                    drop_train_remainder=False)
    ...
    sampled_model_index = 0
    for epoch in range(total_epochs):
        model.precond_update(ds_M, train_full_size, K_batches=K_batches, full_bayesian=full_bayesian,
                             precond_type=precond_type, second_moment_centered=second_moment_centered)
        batch_index = 0
        for img_batch, label_batch in ds_train:
            batch_index = batch_index + 1
...
def MCEM_sampler_UCI(model, dataset_name='boston', batch_size=200, data_dir='./data/',
            ...
            for img_batch, label_batch in ds_train:
                ...
                if epoch < start_sampling_epoch: # fixed learning rate, zero temperature
                    ...
                    if is_end: # sampling the model
                        ...
                        test_log_p, test_se = model.eval_log_likelihood_and_se(ds_test)
                        log_p.append(test_log_p - tf.math.log(Y_std))
                        mse.append(test_se * Y_std**2)
            # print sampling process
            if (epoch + 1) % print_epoch_cycle == 0:
                train_log_p, train_se = model.eval_log_likelihood_and_se(ds_train)
...
def MCEM_sampler_classification(model, dataset_name='boston', batch_size=200, data_dir='./tensorflow_datasets/',
                                lr_0=0.01, momentum_decay=0.9,
                                precond_type='identity', K_batches=None, second_moment_centered=None,
                                resample_in_cycle_head=True, start_sampling_epoch=2000, epochs_per_cycle=50):
    if precond_type != 'identity' and K_batches is None and second_moment_centered is None:
    ...
    def sampler(num_samples=100, print_epoch_cycle=100):
        ...
        sampled_model_index = 0
        for epoch in range(total_epochs):
            model.precond_update(ds_M, train_full_size, precond_type=precond_type, full_bayesian=False,
                                 K_batches=K_batches, second_moment_centered=second_moment_centered)
            batch_index = 0
            for img_batch, label_batch in ds_train:
                batch_index = batch_index + 1
                if epoch < start_sampling_epoch: # fixed learning rate, zero temperature
                    model.sgmcmc_update(img_batch, label_batch, train_full_size, lr=lr_0, momentum_decay=momentum_decay,
                                             resample_moments=False, temperature=0., full_bayesian=False)
                else: # cyclical learning rate, non-zero temperature
                    step_index = (epoch - start_sampling_epoch) * iterations_per_epoch + batch_index
                    step_rate, is_end = cyclical_step_rate(step_index, cycle_length, schedule='cosine', min_value=0.)
                    ...
                    if resample_in_cycle_head == True:
                        ...
                        is_new_cycle = False
                    model.sgmcmc_update(img_batch, label_batch, train_full_size, lr=lr, momentum_decay=momentum_decay,
                                             resample_moments=is_new_cycle, temperature=1., full_bayesian=False)
                    if is_end: # sampling the model
                        sampled_model_index += 1
                        W_samples.append(model.W_mcmc) # W_mcmc is a list
...
def MCEM(sampler_EM, maximizer, sampler_fixing_hyper, total_EM_steps, ds_train,
    ...
    _, log_p, mse_or_acc = sampler_fixing_hyper(num_samples=num_samples_fixing_hyper,
                                          print_epoch_cycle=print_epoch_cycle_fixing)
    return log_p, mse_or_acc #[S, N] or [S,]
...
def MCEM_windows(sampler_EM, maximizer, sampler_fixing_hyper, total_EM_steps, ds_train,
    ...
    for x_batch, y_batch in ds_train_repeat:
        ...
        # E step
        print("#"*15, f"EM step {em_step} of total {total_EM_steps} steps. E Step: ", "#"*15)
        W_samples, log_p, mse_or_acc = sampler_EM(num_samples=1, print_epoch_cycle=print_epoch_cycle_EM)
        W_samples_window.extend(W_samples)
        if len(W_samples_window) == 1:
            log_p_window = log_p
            mse_or_acc_window = mse_or_acc #[S, N] or [S,]
        elif len(W_samples_window) <= window_size:
            log_p_window = tf.concat([log_p_window, log_p],axis=0)
            mse_or_acc_window = tf.concat([mse_or_acc_window, mse_or_acc], axis=0) #[S, N] or [S,]
        else:
            W_samples_window = W_samples_window[-window_size:]
            log_p_window = tf.concat([log_p_window, log_p], axis=0)[1:,:]
            mse_or_acc_window = tf.concat([mse_or_acc_window, mse_or_acc], axis=0)[1:] #[S, N] or [S,]

        n_models = tf.shape(mse_or_acc_window)[0]
        ...
        if tf.rank(mse_or_acc_window) == 1:
            predict_rmse_or_acc = tf.reduce_mean(mse_or_acc_window) # acc
            print(f"Test Mean Acc of all models in window: {predict_rmse_or_acc}\n")
        else:
            predict_rmse_or_acc = tf.math.sqrt(tf.reduce_mean(mse_or_acc_window)) # rmse
            print(f"Test Root MSE of all models in window: {predict_rmse_or_acc}\n")
        # M step
        print("#"*15, f"EM step {em_step} of total {total_EM_steps} steps, M Step: ", "#"*15)
