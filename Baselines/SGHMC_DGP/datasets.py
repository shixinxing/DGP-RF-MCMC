...
class Dataset(object):
    ...
    def get_data(self, seed=0, split=0, prop=0.8):
    ...
    def normalize(self, split_data, X_or_Y):
        ...
        if X_or_Y == "X":
            s = np.std(split_data[X_or_Y], 0)[None, :] + 1e-6  # use the std of the training data, not the test data
        elif X_or_Y == "Y":  # Y and Ys do not divide std
            # s = 1.
        ...
        if X_or_Y == "Y":
            split_data.update({X_or_Y + '_std': s.flatten()})  # Y and Ys do not have std
        return split_data
...
class Boston(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'boston', 506, 13
...
class Kin8mn(Dataset):
    ...
    def download_data(self):
        ...
        data = pandas.read_csv(url, header=0).values
...
class Naval(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'naval', 11934, 16
        self.type = 'regression'
...
class WineWhite(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'wine_white', 4898, 11
        self.type = 'regression'
