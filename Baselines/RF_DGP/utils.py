...
def get_flags():
    ...
    flags.DEFINE_integer('n_iterations', 60001, 'Number of iterations (batches) to feed to the DGP')
    flags.DEFINE_integer('display_step', 500, 'Display progress every FLAGS.display_step iterations')
    flags.DEFINE_integer('mc_train', 60, 'Number of Monte Carlo samples used to compute stochastic gradients')
    flags.DEFINE_integer('mc_test', 60, 'Number of Monte Carlo samples for predictions')
    flags.DEFINE_integer('n_rff', 100, 'Number of random features for each layer')
    flags.DEFINE_integer('df', 13, 'Number of GPs per hidden layer')
    ...
    flags.DEFINE_boolean('feed_forward', True, 'Feed original inputs to each layer')
    ...
    tf.app.flags.DEFINE_string("dataset", "boston", "Dataset name")
